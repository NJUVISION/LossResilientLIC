...
class ProgDTD(ScaleHyperprior):
    ...
    def compress(self, x):
        ...
        #     # if len(ith_channel_string[0]) > 7000 - 4:
        #     #     print(len(ith_channel_string[0]))
        #     assert len(ith_channel_string[0]) <= 7000 - 4
    ...
    def decompress(self, strings, shape, drop_channel_index=[]):
        ...
        if self.loss_packet:
            # y_hat_p = self.random_channel_zeroing_prog(self.p_latent, y_hat_p)
            # random drop channel
            # 随机生成drop_channel个通道
