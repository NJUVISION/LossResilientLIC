...
class LRLICConvNextWoProg(ScaleHyperprior):
    ...
    def random_channel_zeroing_prog(self, p, tensor, zero_prob_vector=[0.005, 0.015, 0.025, 0.035, 0.05]):
    ...
    def decompress(self, strings, shape):
        ...
        if self.loss_packet:
            # random drop channel
            ratio = self.packet_loss_ratio
            drop_channel = int(y_hat_p.shape[1] * ratio)
    ...
    def final_packets(self, packet_channel_combination, y, indexes, means_hat, max_packet_length):
        ...
        for idx, one_packet_channel in enumerate(packet_channel_combination):
            ...
            if(start_channel + count_channel) % 4 == 0 and (start_channel + count_channel) != the_last_channel and count_channel != 1:
