...
class LRLICWProg(ScaleHyperprior):
    ...
    def get_template_channel_bytes(self, y, index, means, q, max_packet_length):
        ...
        x_fit = np.linspace(0, zero_info_channel - 1, zero_info_channel) # only generate channel which contain information
    ...
    def final_packets(self, packet_channel_combination, y, indexes, means_hat, max_packet_length):
        ...
        print(packet_channel_combination)
    ...
    def compress_packets(self, x, max_packet_length, quality):
        ...
        indexes = self.gaussian_conditional.build_indexes(scales_hat)
        channel_lengths = self.get_template_channel_bytes(y_copy, indexes, means_hat, quality, max_packet_length)
        if self.p_latent != 1:
