...
class LRLICWoProg(ScaleHyperprior):
    ...
    def random_channel_meaning_prog(self, p, tensor, means_hat,  zero_prob_vector=[0.005, 0.015, 0.025, 0.035, 0.05]):
        rand_num = np.random.rand()
        if rand_num <= 0.5:
            mask = torch.ones(tensor.shape[0]).to(tensor.device)
            return tensor, mask
            ...
            if p != 1:
                ...
                random_array = np.concatenate((random_array, random_array_replace), axis=0)
                tensor_copy = tensor.clone()
                tensor_copy[random_array < zero_prob, :, :] = means_hat[random_array < zero_prob, :, :]
                mask = torch.from_numpy(random_array >= zero_prob).float()
                return tensor_copy, mask.to(tensor.device)
            else:
                zero_prob = np.random.choice(zero_prob_vector)
    ...
    def forward(self, x):
        ...
        if self.loss_packet:
            ...
            for i in range(y_hat.shape[0]):
                y_hat_random_zero[i, :, :, :], mask = self.random_channel_zeroing_prog(1, y_hat[i, :, :, :])
                # y_hat_random_zero[i, :, :, :], mask = Gilbert_Elliott_Channel_Zeroing(1, y_hat[i, :, :, :])
                # y_hat_random_zero[i, :, :, :], mask = self.random_channel_meaning_prog(1, y_hat[i, :, :, :], means_hat[i, :, :, :])
                batch_mask[i, :] = mask
    ...
    def decompress(self, strings, shape):
        ...
        if self.loss_packet:
            # random drop channel
            # drop_channel_index = [18,41,52,85,103,111,113,117,120,126,141,144,148,151,155,162,179,180,182,183]
            drop_channel_index = []
    ...
    def compress_packets(self, x, max_packet_length, quality):
        ...
        mean_scales_hat = scales_hat.mean(dim=(2, 3))
        std_y_hat = y.std(dim=(2, 3))
        sorted_indices = torch.argsort(mean_scales_hat, dim=1, descending=True)
        print(sorted_indices)
        print(torch.argsort(std_y_hat, dim=1, descending=True))
